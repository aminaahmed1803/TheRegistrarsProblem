...
if (!$ARGV[0] || !$ARGV[1] || !$ARGV[2] || !$ARGV[3] || !$ARGV[4] || !$ARGV[5] || !$ARGV[6] || !$ARGV[7]) {
	...
	print "Usage:\n";
	print "$0: <number of rooms> <number of classes> <number of class times> <number of students> <number of teachers> <number of classes per students> <contraint file> <student prefs file>\n";
	exit 1;
}
...
my $numslots = $ARGV[2];
# print "Number of timeslots is $numslots\n";
if ($numclasses % 2 != 0) {
...
# print "Number of students is $numstudents \n";
# my $numteachers = $numclasses / 2;
my $numteachers = $ARGV[4];
# print "Number of teachers is $numteachers \n";
my $classesperstudent = $ARGV[5]; #4
# print "Number of classes per student is $classesperstudent \n";
my $constraintfile = $ARGV[6];
my $prefsfile = $ARGV[7];
